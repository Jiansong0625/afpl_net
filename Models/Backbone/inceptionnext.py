...
try:
    ...
    from timm.models.layers import trunc_normal_, DropPath  # 旧路径兼容
    try:
...
class InceptionDWConv2d(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels=None, square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125, stride=1):
        super().__init__()
        # 目标通道数（分支卷积与split按目标通道构建）
        target_c = out_channels if out_channels is not None else in_channels
        self.need_proj = (in_channels != target_c) or (stride != 1)
        # 先用1x1完成变通道/下采样，分支卷积始终在target_c上运行
        self.proj = nn.Conv2d(in_channels, target_c, kernel_size=1, stride=stride, bias=False) if self.need_proj else nn.Identity()

        gc = max(1, int(target_c * branch_ratio))  # 每个分支的通道数，至少为1
        branch_stride = 1 if self.need_proj else stride  # 若已在proj中下采样，这里保持stride=1

        self.dwconv_hw = nn.Conv2d(gc, gc, square_kernel_size, stride=branch_stride, padding=square_kernel_size//2, groups=gc)
        self.dwconv_w  = nn.Conv2d(gc, gc, kernel_size=(1, band_kernel_size), stride=branch_stride, padding=(0, band_kernel_size//2), groups=gc)
        self.dwconv_h  = nn.Conv2d(gc, gc, kernel_size=(band_kernel_size, 1), stride=branch_stride, padding=(band_kernel_size//2, 0), groups=gc)
